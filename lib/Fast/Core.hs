...
-- | display the config
instance P.Show FastConfig where
  show c =
    T.printf
...
-- | constructs a default FastConfig
--
-- configHost:
--
-- @http://localhost@
...
addAuthMethod :: AuthMethod auth => FastConfig -> auth -> FastConfig
addAuthMethod config@FastConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: FastConfig -> IO FastConfig
...
withStderrLogging :: FastConfig -> IO FastConfig
...
withNoLogging :: FastConfig -> FastConfig
...
data FastRequest req contentType res accept = FastRequest
  { rMethod  :: NH.Method   -- ^ Method of FastRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of FastRequest
  , rParams   :: Params -- ^ params of FastRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (FastRequest req contentType res accept) NH.Method
rMethodL f FastRequest{..} = (\rMethod -> FastRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (FastRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f FastRequest{..} = (\rUrlPath -> FastRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (FastRequest req contentType res accept) Params
rParamsL f FastRequest{..} = (\rParams -> FastRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (FastRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f FastRequest{..} = (\rAuthTypes -> FastRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => FastRequest req contentType res accept -> param -> FastRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: FastRequest req contentType res accept -> param -> FastRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: FastRequest req contentType res accept -> param -> FastRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
_mkRequest :: NH.Method -- ^ Method
          ...
          -> FastRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = FastRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader ::
     FastRequest req contentType res accept
  -> [NH.Header]
  -> FastRequest req contentType res accept
...
removeHeader :: FastRequest req contentType res accept -> [NH.HeaderName] -> FastRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => FastRequest req contentType res accept -> FastRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => FastRequest req contentType res accept -> FastRequest req contentType res accept
...
addForm :: FastRequest req contentType res accept -> WH.Form -> FastRequest req contentType res accept
addForm req newform =
...
_addMultiFormPart :: FastRequest req contentType res accept -> NH.Part -> FastRequest req contentType res accept
...
_setBodyBS :: FastRequest req contentType res accept -> B.ByteString -> FastRequest req contentType res accept
_setBodyBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: FastRequest req contentType res accept -> BL.ByteString -> FastRequest req contentType res accept
_setBodyLBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => FastRequest req contentType res accept -> P.Proxy authMethod -> FastRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    ...
    -> a
    -> FastRequest req contentType res accept
    -> IO (FastRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> FastConfig
  -> IO (FastRequest req contentType res accept)
_applyAuthMethods req config@(FastConfig {configAuthMethods = as}) =
  foldlM go req as
  where
